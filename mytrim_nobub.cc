...
// argv = pointer to supplied arguments
{
  ...
  double hashr = length/10; //fix for no bubble
  
  sample->initSpatialhash( int( sample->w[0] / hashr ) - 1,
                          int( sample->w[1] / hashr ) - 1,
                          int( sample->w[2] / hashr ) - 1 );
  ...
  for( int n = 1; n <= fissions; n++ )
  {
    ...
    if (runtrim == true)
    {
      ...
      while( !recoils.empty() )
      {
        ...
        if( pka->tag >= 0 )
        {
          ...
          for( int i = 0; i < 3; i++ ) dif[i] = sample->c[i][pka->tag] - pka->pos[i];
    ...
    if( save_fsnFile )
    {
      ...
      fprintf(fsnFile, "%li\t%i\t%i\t%i\n", simconf->ionId - oldionId, hitNum, escNum, mixNum);
    }
